"""LLM system-prompt string constants.

Defines the system prompts that steer the model's answering behavior and
collects them, in order, into ``MODEL_SYSTEM_PROMPTS`` so callers can
select one by index (see ``model_adapters.get_system_prompt``).

The ``DEEP_REPORTING_V1``/``V2``/``V3`` prompts are successive revisions of
an evidence-classification-and-structured-answer template; later versions
add classification categories and stricter rules. ``SMOKING_MAN`` is an
alternative "intelligence analyst" persona prompt.
"""
