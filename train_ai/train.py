...
class TrainingConfig:
    ...
    def __post_init__(self) -> None:
        ...
        Raises:
            ValueError: If ``mode`` is not ``"corpus"`` or ``"qa"``, or if
                ``eval_split_frac`` is not in the half-open range [0, 1).
        """
...
def _resolve_dtype(name: str):
    ...
    Args:
        name: One of ``"auto"``, ``"bfloat16"``, ``"float16"``, ``"float32"``.

    Returns:
        The matching ``torch`` dtype, or the string ``"auto"`` for the
        ``"auto"`` option or any unrecognized name.
    """
    import torch
    return {
        "auto": "auto",
...
def _build_parser() -> argparse.ArgumentParser:
    """Build the command-line argument parser for the trainer.
...
def _configure_logging(level: str) -> None:
    ...
    Args:
        level: Log level name (e.g. ``"INFO"``); unrecognized values fall
            back to ``INFO``.
    """
    logging.basicConfig(
...
def main(argv: Optional[List[str]] = None) -> int:
    """CLI entry point: parse args, build a TrainingConfig, and train.
