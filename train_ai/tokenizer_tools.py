...
def _build_parser() -> argparse.ArgumentParser:
    """Build the argument parser with the three tool subcommands.

    Defines the ``add-stop``, ``rewrite-qa-stop``, and ``convert-gguf``
...
def main(argv: Optional[List[str]] = None) -> int:
    """CLI entry point: dispatch to the selected ``--tool`` subcommand.

    Routes to ``add_stop_token``, ``rewrite_qa_stop_token``, or
    ``convert_gguf_to_hf`` based on the chosen subcommand.
