...
@dataclass
class GenConfig:
    """Configuration for the QA-pair generation pipeline.

    Holds the model to load, the corpus and output directories, the
    text-windowing shape, volume/checkpointing limits, generation
    sampling settings, and model-load options used by
    ``generate_qa_pairs``.
    """
...
def _build_parser() -> argparse.ArgumentParser:
    """Build the command-line argument parser for QA-pair generation.
...
def main(argv: Optional[List[str]] = None) -> int:
    """CLI entry point: parse args, build a GenConfig, and generate QA pairs.
