...
def _is_probable_page_number(text: str, bbox, page_w: float, page_h: float) -> bool:
    """Heuristically decide whether a block is a running page number.

    True when the text is a short run of digits and its bounding box is
    horizontally centered and sits in the top or bottom margin band.

    Args:
        text: The block's text.
        bbox: The block bounding box ``(x0, y0, x1, y1)``.
        page_w: Page width in points.
        page_h: Page height in points.

    Returns:
        True if the block looks like a page number, else False.
    """
    text = text.strip()
...
def _normalize_inline_text(block) -> str:
    """Flatten a PyMuPDF text block into a single whitespace-normalized string.

    Concatenates every span across every line, collapsing runs of
    whitespace and joining the pieces with single spaces.

    Args:
        block: A PyMuPDF "dict"-mode text block.

    Returns:
        The block's text as one inline string.
    """
    parts = []
...
def _page_median_font(pdict) -> float:
    """Return the median span font size across all blocks on a page.

    Args:
        pdict: A PyMuPDF page dict (from ``page.get_text("dict")``).

    Returns:
        The median font size in points, or 10.0 if the page has no sized
        spans.
    """
    sizes = []
...
def _block_avg_font(block) -> float:
    """Return the average span font size within a single text block.

    Args:
        block: A PyMuPDF "dict"-mode text block.

    Returns:
        The mean font size in points, or 0.0 if the block has no sized
        spans.
    """
    sizes = []
...
def _should_drop_block(block, page_w: float, page_h: float, med_font: float,
    ...
    """Decide whether a text block should be discarded from the output.

    A block is dropped if it is empty, a probable page number, a learned
    repeated header/footer, or a small-font / footnote-marked block sitting
    ...
    Args:
        ...
        page_w: Page width in points.
        page_h: Page height in points.
        med_font: The page's median font size, for relative comparison.
        headers: Learned repeated header strings to drop.
        footers: Learned repeated footer strings to drop.

    Returns:
        True if the block should be dropped, else False.
    """
...
def _sort_blocks_reading_order(blocks):
    """Sort blocks top-to-bottom then left-to-right into reading order.

    Args:
        blocks: An iterable of PyMuPDF text blocks (each with a ``bbox``).

    Returns:
        A new list of the blocks sorted by rounded ``(y0, x0)``.
    """
    return sorted(blocks, key=lambda b: (round(b["bbox"][1], 1), round(b["bbox"][0], 1)))


def _lines_from_block(block):
    """Extract the text of each line in a block as a list of strings.

    Args:
        block: A PyMuPDF "dict"-mode text block.

    Returns:
        One string per line, each the concatenation of its spans' text.
    """
    lines = []
    for ln in block.get("lines", []):
...
def _dehyphenate_and_reflow(lines) -> str:
    """Join wrapped lines into paragraphs, repairing hyphenation.

    Consecutive non-empty lines are merged into a single paragraph; a
    trailing hyphen followed by a lowercase continuation is de-hyphenated;
    list-marker lines (bullets, numbered items) start a new paragraph; and
    blank lines act as paragraph boundaries. Paragraphs are separated by
    blank lines in the output.

    Args:
        lines: The per-line strings to reflow, with empty strings marking
            paragraph breaks.

    Returns:
        The reflowed text with paragraphs separated by blank lines.
    """
    out = []
    buf = ""
    for line in lines:
