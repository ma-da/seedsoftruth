...
def _connect(db_path: str) -> sqlite3.Connection:
    """Open a SQLite connection to `db_path` tuned for the crawler.

    The connection is configured with ``journal_mode=WAL`` and
    ``busy_timeout=5000`` so concurrent workers avoid "database is locked"
    errors.
...
def table_exists(db_path: str, table_name: str) -> bool:
    """Check whether `table_name` exists in the cache database.
    ...
    Returns:
        True if the table exists; False if it is absent or on any
        SQLite error.
    """
    try:
        with _connect(db_path) as conn:
...
def remove_download_entry(db_path: str, cleaned_url: str) -> None:
    """Delete the downloads-table row for `cleaned_url`.

    Errors are logged and swallowed rather than raised.

    Args:
        db_path: Path to the SQLite database file.
        cleaned_url: The cleaned URL key whose cache row should be removed.
    """
    try:
        with _connect(db_path) as conn:
...
def save_pending_url(db_path: str, url: str,
    ...
    """Persist a pending URL (with its depths) into the url_queue table.

    Uses ``INSERT OR IGNORE`` so re-enqueuing an already-pending URL is a
    no-op.

    Args:
        db_path: Path to the SQLite database file.
        url: The URL to mark as pending.
        depth_actual: Actual hop count from the start URL.
        depth_effective: Effective depth (reset to 0 on home-domain pages).
    """
    with _connect(db_path) as conn:
        conn.execute("""
...
def delete_pending_url(db_path: str, url: str) -> None:
    """Remove `url` from the pending url_queue table once it is processed.

    Args:
        db_path: Path to the SQLite database file.
        url: The URL to delete from the pending queue.
    """
    with _connect(db_path) as conn:
...
def clear_pending_queue(db_path: str) -> None:
    """Delete every row from the pending url_queue table.
