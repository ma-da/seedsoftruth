...
--------------
    ...
    main(argv)               CLI entry point. Invoke via ``python3 ./web_scraper.py``.
...
import _scraper_cache as cache
import _scraper_pdf as pdf
...
class ScraperConfig:
    ...
    # Infinite-scroll handling. When ``scroll`` is True and the fetch
    # strategy is "playwright", every HTML page is scrolled to the bottom
    # repeatedly until the document height stops growing (or a budget is
    # exhausted). Use this for sites whose listing pages lazy-load older
    # content via JS (Substack archive, Medium tag pages, etc.). Article
    # pages don't lazy-load so scrolling them is a brief no-op.
    scroll: bool = False
    scroll_max_hops: int = 40        # safety cap on scroll iterations per page
    scroll_quiet_ms: int = 1500      # ms to wait after each scroll for content to load
...
class CrawlResult:
    ...
    Attributes:
        num_pages_visited: Total number of pages the crawl visited.
    """
...
def _should_visit(url: str, visited: Set[str], deny_substrings: List[str],
    ...
    """Decide whether `url` is eligible to be crawled.

    Rejects generic skips (mailto/javascript/image assets), already-visited
    URLs, ``web.archive.org`` hosts, and any URL matching a configured deny
    substring or regex.

    Args:
        url: The candidate URL.
        visited: Set of URLs already visited.
        deny_substrings: Substrings that disqualify a URL if contained.
        deny_patterns: Compiled regexes that disqualify a URL if matched.

    Returns:
        True if the URL should be visited, else False.
    """
...
def crawl_site(config: ScraperConfig) -> CrawlResult:
    ...
    def add_url_to_crawl(url: str, depth_actual: int, depth_effective: int) -> None:
        """Enqueue a URL for crawling and persist it to the pending queue.

        Args:
            url: The URL to enqueue.
            depth_actual: Actual hop count from the start URL.
            depth_effective: Effective depth (reset on home-domain pages).
        """
    ...
    def finalize_url(url: str) -> None:
        """Mark `url` as done by removing it from the persisted pending queue.

        Args:
            url: The URL that has finished processing.
        """
    ...
    def fetch_with_ratelimit(url: str, headers: dict):
        ...
        if config.fetch_strategy == "playwright":
            result = fetcher.fetch(url, headers=headers,
                                   ...
                                   scroll_max_hops=config.scroll_max_hops,
                                   scroll_quiet_ms=config.scroll_quiet_ms)
        else:
            result = fetcher.fetch(url, headers=headers,
    ...
    def crawl(url: str, depth_actual: int, depth_effective: int) -> None:
        """Fetch, save, and (for home pages) expand one URL.

        Fetches the URL with rate-limit handling, enforces the max-pages
        limit and progress reporting, then dispatches by content type:
        broken links go to the Wayback fallback, PDFs are extracted, and
        HTML is parsed, deduplicated by content hash, saved, and (when the
        page is in a home domain and under ``max_depth``) has its child
        links enqueued.

        Args:
            url: The URL to crawl.
            depth_actual: Actual hop count from the start URL.
            depth_effective: Effective depth; reset to 0 for home-domain
                pages so the depth budget only limits travel away from home.
        """
    ...
    def worker(worker_id: int) -> None:
        """Worker-thread loop: pull URLs off the queue and crawl them.

        Runs until ``stop_event`` is set, pulling ``(url, depth_actual,
        depth_effective)`` tuples from the shared queue. Exceptions from a
        single URL are logged and swallowed so one bad page can't kill the
        worker; each URL is finalized and marked done in a ``finally``.

        Args:
            worker_id: 1-based identifier used in log messages.
        """
...
def _save_html(cleaned_url: str, soup_bytes: bytes, content_hash: str,
    ...
    """Write a page's prettified HTML and extracted text, and cache it.

    Writes ``<cleaned_url>.html`` and ``<cleaned_url>.txt`` under the
    output dir and, when caching is enabled, records both files' paths,
    sizes, and the content hash in the downloads table. No-op if
    ``save_html_content`` is disabled.

    Args:
        cleaned_url: The cleaned URL, used to derive output filenames.
        soup_bytes: Prettified HTML bytes to write.
        content_hash: Hash of the HTML content for the cache entry.
        config: The active scraper configuration.
    """
...
def _handle_pdf(url: str, cleaned_url: str, config: ScraperConfig) -> None:
    """Download a PDF and write its extracted clean text alongside it.

    Saves the PDF as ``<cleaned_url>.pdf`` and, on successful extraction,
    its title + text as the matching ``.txt``. Download or extraction
    failures (including a missing PyMuPDF) are logged and skipped.
...
def _handle_broken_link(url: str, status_code: int, config: ScraperConfig,
                        headers: dict) -> None:
    """Try to recover a broken link from the Wayback Machine.

    Looks up the closest archived snapshot of `url`; if one exists, fetches
    it and writes the raw bytes to an ``archived_<key>`` file in the output
    dir. Does nothing if no snapshot is found or the fetch fails.

    Args:
        url: The URL that returned a non-200 status.
        status_code: The status code that flagged the link as broken.
        config: The active scraper configuration.
        headers: Request headers to use for the archive fetch.
    """
...
def _build_parser() -> argparse.ArgumentParser:
    """Build the command-line argument parser for the crawler.

    Returns:
        A configured ``argparse.ArgumentParser`` for the CLI.
    """
    ...
    p.add_argument("--scroll", action="store_true",
                   help="Scroll each HTML page to the bottom until content "
                        "stops loading (handles infinite-scroll listings like "
                        "Substack /archive). Requires playwright.")
    p.add_argument("--scroll-max-hops", type=int, default=None,
                   help="Safety cap on scroll iterations per page (default 40).")
    p.add_argument("--scroll-quiet-ms", type=int, default=None,
                   help="ms to wait after each scroll for new content (default 1500).")
...
def _configure_logging(level: str, log_file: Optional[str]) -> None:
    ...
    Sets the root level and attaches a formatted stderr handler; if
    `log_file` is given, also adds a file handler (mode ``"w"``), creating
    the parent directory as needed.

    Args:
        level: Log level name (e.g. ``"INFO"``).
        log_file: Optional path to also write logs to; None for stderr only.
    """
...
def _normalize_url(url: str) -> str:
    """Ensure `url` has an http(s) scheme, prefixing ``http://`` if missing.

    Args:
        url: The URL, possibly schemeless.

    Returns:
        The URL with a scheme guaranteed.
    """
...
def main(argv: Optional[List[str]] = None) -> int:
    """CLI entry point: parse args, build the config, and run the crawl.

    Resolves a YAML ``--profile`` if given, applies CLI overrides, and
    requires ``--start-url`` when no profile is supplied.

    Args:
        argv: Argument list to parse; defaults to ``sys.argv[1:]``.

    Returns:
        Process exit code (0 on success, 2 if ``--start-url`` is missing).
    """
    ...
    if args.scroll:
        overrides["scroll"] = True
    if args.scroll_max_hops is not None:
        overrides["scroll_max_hops"] = args.scroll_max_hops
    if args.scroll_quiet_ms is not None:
        overrides["scroll_quiet_ms"] = args.scroll_quiet_ms
