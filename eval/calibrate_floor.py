...
def fmt_top1(t: Any) -> str:
    """Format a top-1 score for display, showing '  --  ' for None/NaN/non-numeric."""
    if t is None:
        return "  --  "
    try:
        x = float(t)
...
def is_declined(top1: Any, n_results: int, threshold: float) -> bool:
    """Whether the gate would decline a query at the given top-1 score threshold.

    Args:
        top1: The top-1 BM25 score (may be None, NaN, or non-numeric).
        n_results: Number of results returned for the query.
        threshold: Minimum top-1 score required to pass the gate.

    Returns:
        True if there are no results, or the top-1 score is missing/NaN/below
...
def main() -> None:
    """Sweep BM25 top-1 score thresholds and report the best gate floor per variant.

    Loads raw records from a prior ab_eval run, splits them into in-domain and
    probe queries per variant, and sweeps thresholds from --threshold-min to
    --threshold-max. For each threshold it computes the true-decline rate (TD),
    false-decline rate (FD), and Youden's J = TD - FD, picks the J-maximizing
    threshold, and prints the per-query score distributions plus which queries
    decline or slip through at the optimum.
    """
