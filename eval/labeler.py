...
def parse_grade(text: str) -> Optional[int]:
    """Extract the first 0–3 relevance grade found in the judge's text response.

    Args:
        text: The raw judge output.

    Returns:
        The grade as an int, or None if no 0–3 digit is present.
    """
...
def judge_anthropic(
    ...
    """Grade a (query, doc) prompt via the Anthropic Messages API.
    ...
    Returns:
        A (grade, raw_text) tuple, where grade is the parsed 0–3 value (or None)
        and raw_text is the stripped model response.
...
def judge_stub(prompt: str) -> Tuple[Optional[int], str]:
    """Return a deterministic fake grade derived from the prompt hash.

    Produces meaningless 0–3 grades; useful only for testing the pipeline
    without an API key.

    Returns:
        A (grade, rationale) tuple.
    """
...
def call_search(
    ...
    """Call /api/search for one query and variant and return its results.

    Args:
        base_url: Base URL of the running Flask app.
        query: The search query.
        variant: rag_algo_type variant to retrieve with.
        top_k: Number of results to request.
        timeout: Request timeout in seconds.

    Returns:
        The list of result dicts (empty if none).

    Raises:
        requests.HTTPError: If the endpoint returns a non-2xx status.
        RuntimeError: If the response body reports ok=False.
    """
...
def doc_id_of(doc: Dict[str, Any], field: str) -> Optional[str]:
    """Extract the doc identifier from a result dict, as a string or None.

    Args:
        doc: A single search result.
        field: The result key holding the identifier.

    Returns:
        The stringified identifier, or None if the field is absent/None.
    """
    v = doc.get(field)
...
def truncate_words(s: str, max_words: int) -> str:
    """Truncate a string to at most max_words, appending an ellipsis if cut.

    Args:
        s: The input string (None is treated as empty).
        max_words: Maximum number of whitespace-separated words to keep.

    Returns:
        The whitespace-normalized string, suffixed with ' …' when truncated.
    """
    words = (s or "").split()
...
def load_cache(path: Path) -> Dict[CacheKey, Tuple[Optional[int], str]]:
    """Load the append-only judge cache into a dict.

    Args:
        path: Path to the judge_cache.jsonl file.

    Returns:
        A mapping of (query, doc_id) -> (grade, rationale). Empty if the file
        does not exist. Malformed lines are skipped; later records for a key
...
def append_cache(
    ...
    """Append a single judgment record to the judge cache file as one JSON line.

    Args:
        path: Path to the judge_cache.jsonl file.
        query: The judged query.
        doc_id: The judged document identifier.
        grade: The 0–3 grade, or None if judging failed.
        rationale: The judge's brief rationale or error message.
    """
...
def main() -> None:
    """Build the A/B eval label set by LLM-judging pooled retrieval candidates.

    Parses CLI arguments and resolves the judge backend (anthropic, deepinfra,
    or stub). Loads seed queries, passes probe queries through unjudged, and for
    each in-domain query pools unique candidate docs across variants via
    /api/search. Each (query, doc) pair is graded 0–3 by the judge, using and
    appending to the on-disk cache so re-runs skip already-judged pairs.
    Copyright-protected docs are graded from metadata only. Writes the
    ab_eval-compatible labels.jsonl and prints run/cache statistics.

    Raises:
        SystemExit: On a missing API key, malformed seed JSON, or no seed queries.
    """
