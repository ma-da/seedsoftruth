...
@dataclass
class Labeled:
    """A single eval-set record: a query plus its relevance labels.

    Attributes:
        query: The search query text.
        labels: Mapping of doc_id -> graded relevance (binary labels use 1).
        id_field: Result field used as the doc identifier (default "row_id").
        probe: True for off-corpus probe queries, judged on declination rather
            than ranking; such records carry empty labels.
    """
...
def load_eval_set(path: Path) -> List[Labeled]:
    """Parse a JSONL eval set into a list of Labeled records.

    Skips blank lines and '#' comment lines. Each record must carry a non-empty
    'query' and one of: 'labels' (a doc_id -> grade dict), 'relevant_doc_ids'
    (a list, treated as binary grade 1), or 'probe: true'. An optional
    'id_field' overrides the default result key to match against.

    Args:
        path: Path to the JSONL eval set.

    Returns:
        The parsed Labeled records in file order.

    Raises:
        SystemExit: On malformed JSON, a missing query, or a record lacking any
            recognized label/probe field.
    """
    items: List[Labeled] = []
    with open(path) as f:
...
def get_doc_id(result: Dict[str, Any], id_field: str) -> Optional[str]:
    ...
    Args:
        ...
        id_field: The result key holding the identifier.
...
def _relevant(labels: Dict[str, int]) -> set:
    """Return the set of doc_ids with a positive (>0) relevance grade."""
    return {d for d, g in labels.items() if g > 0}


def recall_at_k(ranked_ids: List[str], labels: Dict[str, int], k: int) -> float:
    """Recall@k: fraction of relevant docs found in the top-k ranked ids.

    Returns NaN when there are no relevant docs.
    """
...
def precision_at_k(ranked_ids: List[str], labels: Dict[str, int], k: int) -> float:
    """Precision@k: fraction of the top-k ranked ids that are relevant.

    Returns NaN when k <= 0 or there are no relevant docs.
    """
    if k <= 0:
        return float("nan")
    rel = _relevant(labels)
...
def mrr(ranked_ids: List[str], labels: Dict[str, int]) -> float:
    """Reciprocal rank of the first relevant doc (0.0 if none ranked).
...
def average_precision(ranked_ids: List[str], labels: Dict[str, int]) -> float:
    ...
    Normalized by the total number of relevant docs. Returns NaN when there are
    no relevant docs.
    """
    rel = _relevant(labels)
    if not rel:
...
def ndcg_at_k(ranked_ids: List[str], labels: Dict[str, int], k: int) -> float:
    """Normalized DCG@k: DCG of the top-k ranking over the ideal DCG.

    Returns NaN when there are no labels or no positive ideal gain.
    """
    if not labels:
        return float("nan")
...
def main() -> None:
    """Run the A/B retrieval eval end to end.

    Parses CLI arguments, loads the eval set, and either calls /api/search for
    every (query, variant) pair or reloads a prior raw_*.jsonl when --no-server
    is set. Computes per-query ranking metrics for in-domain queries and
    declination metrics for probe queries, prints aggregate tables and pairwise
    comparisons (paired bootstrap CIs, Wilcoxon p-values, Holm-adjusted), and
    writes the raw records and per-query metrics CSV to the output directory.

    Raises:
        SystemExit: On invalid arguments or an empty eval set / reused raw file.
    """
