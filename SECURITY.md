...
Seeds of Truth is **alpha software**. The reference deployment is a private invite-only service; the codebase has not been hardened for the open internet. There are known security gaps (documented under *Project status* in the [README](readme.md)) that an operator must close before exposing this to untrusted users. Read those before deploying.
...
- **Reports of "no rate limit on `/api/unlock`," "no CSRF on POST endpoints," or similar already-known alpha gaps** (see *Project status* in the README). We know. They are tracked. PRs welcome.
